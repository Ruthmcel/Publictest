view: sales_eps_tx_tp {
  ...
  dimension: unique_key {
    hidden: yes
    primary_key: yes
    sql: ${chain_id} ||'@'|| ${nhin_store_id} ||'@'|| ${tx_tp_id} ||'@'|| ${sales.sold_flg} ||'@'|| ${sales.adjudicated_flg} ||'@'|| ${report_calendar_global.type} ;; #ERXLPS-1649
  }
  ...
  measure: sum_sales_tx_tp_tp_paid_amount {
    ...
    description: "Total amount paid or to be paid by the third party"
    type: sum
    sql: CASE WHEN (${sales.adjudicated_flg} = 'Y' or ${sales.sold_flg} = 'Y') AND ${report_calendar_global.type} = 'TY' THEN ${TABLE}.TX_TP_TP_PAID_AMOUNT END ;;
  ...
  measure: sum_sales_tx_tp_submit_amount {
    ...
    description: "Total expected third party pay amount submitted to the third party"
    type: sum
    sql: CASE WHEN (${sales.adjudicated_flg} = 'Y' or ${sales.sold_flg} = 'Y') AND ${report_calendar_global.type} = 'TY' THEN ${TABLE}.TX_TP_SUBMIT_AMOUNT END ;;
    drill_fields: [sales.sales_transaction_level_drill_path*]
    value_format: "$#,##0.00;($#,##0.00)"
  }
  ...
  measure: sum_sales_tx_tp_final_compound_fee {
    ...
    description: "Total final transaction third party compounding fee"
    type: sum
    sql: CASE WHEN (${sales.adjudicated_flg} = 'Y' or ${sales.sold_flg} = 'Y') AND ${report_calendar_global.type} = 'TY' THEN ${TABLE}.TX_TP_FINAL_COMPOUND_FEE END ;;
    drill_fields: [sales.sales_transaction_level_drill_path*]
    value_format: "$#,##0.00;($#,##0.00)"
  }
  ...
  measure: sum_sales_tx_tp_final_cost {
    ...
    description: "Final transaction third party cost"
    type: sum
    sql: CASE WHEN (${sales.adjudicated_flg} = 'Y' or ${sales.sold_flg} = 'Y') AND ${report_calendar_global.type} = 'TY' THEN ${TABLE}.TX_TP_FINAL_COST END ;;
    drill_fields: [sales.sales_transaction_level_drill_path*]
    value_format: "$#,##0.00;($#,##0.00)"
  }
  ...
  measure: sum_sales_tx_tp_final_upcharge {
    ...
    description: "Total final transaction third party upcharge"
    type: sum
    sql: CASE WHEN (${sales.adjudicated_flg} = 'Y' or ${sales.sold_flg} = 'Y') AND ${report_calendar_global.type} = 'TY' THEN ${TABLE}.TX_TP_FINAL_UPCHARGE END ;;
    drill_fields: [sales.sales_transaction_level_drill_path*]
    value_format: "$#,##0.00;($#,##0.00)"
  }
  ...
  measure: sum_sales_tx_tp_final_incentive_fee {
    ...
    description: "Total final transaction third party incentive fee"
    type: sum
    sql: CASE WHEN (${sales.adjudicated_flg} = 'Y' or ${sales.sold_flg} = 'Y') AND ${report_calendar_global.type} = 'TY' THEN ${TABLE}.TX_TP_FINAL_INCENTIVE_FEE END ;;
    drill_fields: [sales.sales_transaction_level_drill_path*]
    value_format: "$#,##0.00;($#,##0.00)"
  }
  ...
  measure: sum_sales_tx_tp_amount_collected_from_tp {
    ...
    type: sum
    sql: CASE WHEN (${sales.adjudicated_flg} = 'Y' or ${sales.sold_flg} = 'Y') AND ${report_calendar_global.type} = 'TY' THEN ${TABLE}.TX_TP_AMOUNT_COLLECTED_FROM_TP END ;;
    drill_fields: [sales.sales_transaction_level_drill_path*]
    value_format: "$#,##0.00;($#,##0.00)"
  }
  ...
  measure: sum_sales_tx_tp_other_insurance_amount {
    ...
    description: "Total amount paid by another third party"
    type: sum
    sql: CASE WHEN (${sales.adjudicated_flg} = 'Y' or ${sales.sold_flg} = 'Y') AND ${report_calendar_global.type} = 'TY' THEN ${TABLE}.TX_TP_OTHER_INSURANCE_AMOUNT END ;;
    drill_fields: [sales.sales_transaction_level_drill_path*]
    value_format: "$#,##0.00;($#,##0.00)"
  }
  ...
  set: sales_tx_tp_dimension_candidate_list {
    fields: [
      ...
      tx_tp_copay_override_reason,
      tx_tp_tp_daw, #ERX-8604 - Adding object for TP DAW NULL reporting
