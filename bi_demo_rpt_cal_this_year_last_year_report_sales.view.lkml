view: bi_demo_rpt_cal_this_year_last_year_report_sales {
  derived_table: {
    sql: SELECT
      ...
      WHERE {% condition bi_demo_rx_tx_active.this_year_last_year_filter %} 'Yes-ReportableSales' {% endcondition %}     -- Will ensure records are fetched from report calendar only when LY analysis is performed on the Reportable Sales Date
      AND {% condition bi_demo_rx_tx_active.reportable_sales_date_filter %} CALENDAR_DATE {% endcondition %}
