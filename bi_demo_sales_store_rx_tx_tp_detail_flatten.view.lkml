view: bi_demo_sales_store_rx_tx_tp_detail_flatten {
  ...
  measure: sum_primary_tx_tp_response_detail_dispensing_fee_paid {
    ...
    type: sum
    sql: CASE WHEN (${bi_demo_sales.adjudicated_flg} = 'Y' or ${bi_demo_sales.sold_flg} = 'Y') THEN ${TABLE}.PRIMARY_TX_TP_RESPONSE_DETAIL_DISPENSING_FEE_PAID END ;;
  ...
  measure: sum_secondary_tx_tp_response_detail_dispensing_fee_paid {
    ...
    type: sum
    sql: CASE WHEN (${bi_demo_sales.adjudicated_flg} = 'Y' or ${bi_demo_sales.sold_flg} = 'Y') THEN ${TABLE}.SECONDARY_TX_TP_RESPONSE_DETAIL_DISPENSING_FEE_PAID END ;;
    value_format: "$#,##0.00;($#,##0.00)"
  }
  ...
  measure: sum_tertiary_tx_tp_response_detail_dispensing_fee_paid {
    ...
    type: sum
    sql: CASE WHEN (${bi_demo_sales.adjudicated_flg} = 'Y' or ${bi_demo_sales.sold_flg} = 'Y') THEN ${TABLE}.TERTIARY_TX_TP_RESPONSE_DETAIL_DISPENSING_FEE_PAID END ;;
    value_format: "$#,##0.00;($#,##0.00)"
  }
}
