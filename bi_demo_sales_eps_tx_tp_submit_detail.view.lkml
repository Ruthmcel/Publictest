view: bi_demo_sales_eps_tx_tp_submit_detail {
  # [ERXLPS-1020] - New view with sales_eps_tx_tp_submit_detail created for eps_tx_tp_submit_detail.
  ...
  dimension: unique_key {
    hidden: yes
    primary_key: yes
    sql: ${chain_id} ||'@'|| ${nhin_store_id} ||'@'|| ${tx_tp_submit_detail_id} ||'@'|| ${bi_demo_sales.sold_flg} ||'@'|| ${bi_demo_sales.adjudicated_flg}  ;; #ERXLPS-1649
  }
  ...
  dimension: tx_tp_submit_detail_prescriber_identifier_deidentified {
    ...
    sql: SHA2(${TABLE}.TX_TP_SUBMIT_DETAIL_PRESCRIBER_IDENTIFIER) ;;
  }

  #[ERXLPS-1436] - Deidentified dimension to use in DEMO Model
  dimension: tx_tp_submit_detail_cardholder_identifier_deidentified {
    ...
    type: string
    # sha2 by default uses 256 as the message digest size.
    sql: SHA2(${TABLE}.TX_TP_SUBMIT_DETAIL_CARDHOLDER_IDENTIFIER) ;;
  }
  ...
  #[ERXLPS-1436] - Deidentified dimension to use in DEMO Model
  dimension: tx_tp_submit_detail_group_identifier_deidentified {
    ...
    type: string
    # sha2 by default uses 256 as the message digest size.
    sql: SHA2(${TABLE}.TX_TP_SUBMIT_DETAIL_GROUP_IDENTIFIER) ;;
  }
  ...
  measure: sum_sales_tx_tp_submit_detail_percent_sales_tax_amount {
    ...
    description: "Percentage sales tax submitted. This field is EPS only!!!"
    type: sum
    sql: CASE WHEN (${bi_demo_sales.adjudicated_flg} = 'Y' or ${bi_demo_sales.sold_flg} = 'Y')  THEN ${TABLE}.TX_TP_SUBMIT_DETAIL_PERCENT_SALES_TAX_AMOUNT END ;;
    value_format: "$#,##0.00;($#,##0.00)"
  }
  ...
  measure: sum_sales_tx_tp_submit_detail_percent_sales_tax_rate {
    ...
    description: "Percentage sales tax rate used to calculate 'Percentage Sales Tax Amount Submitted' (482-GE). This field is EPS only!!!"
    type: sum
    sql: CASE WHEN (${bi_demo_sales.adjudicated_flg} = 'Y' or ${bi_demo_sales.sold_flg} = 'Y')  THEN ${TABLE}.TX_TP_SUBMIT_DETAIL_PERCENT_SALES_TAX_RATE END ;;
    value_format: "00.00\"%\""
  }
  ...
  measure: sum_sales_tx_tp_submit_detail_quantity_dispensed {
    ...
    description: "Quantity dispensed expressed in metric decimal units. This field is EPS only!!!"
    type: sum
    sql: CASE WHEN (${bi_demo_sales.adjudicated_flg} = 'Y' or ${bi_demo_sales.sold_flg} = 'Y')  THEN ${TABLE}.TX_TP_SUBMIT_DETAIL_QUANTITY_DISPENSED END ;;
    value_format: "#,##0.00"
  }
  ...
  measure: sum_sales_tx_tp_submit_detail_days_supply {
    ...
    description: "Estimated number of days the prescription will last. This field is EPS only!!!"
    type: sum
    sql: CASE WHEN (${bi_demo_sales.adjudicated_flg} = 'Y' or ${bi_demo_sales.sold_flg} = 'Y')  THEN ${TABLE}.TX_TP_SUBMIT_DETAIL_DAYS_SUPPLY END ;;
    value_format: "#,##0.00"
  }
  ...
  measure: sum_sales_tx_tp_submit_detail_uc_charge {
    ...
    description: "Total amount charged cash customers for the prescription exclusive of sales tax or other amounts claimed. This field is EPS only!!!"
    type: sum
    sql: CASE WHEN (${bi_demo_sales.adjudicated_flg} = 'Y' or ${bi_demo_sales.sold_flg} = 'Y')  THEN ${TABLE}.TX_TP_SUBMIT_DETAIL_UC_CHARGE END ;;
    value_format: "$#,##0.00;($#,##0.00)"
  }
  ...
  measure: sum_sales_tx_tp_submit_detail_gross_amount_fee {
    ...
    description: "Total price claimed from all sources. For prescription claim request, field represents a sum of Ingredient Cost Submitted (49-D9), Dispensing Fee Submitted (412-DC), Flat Sales Tax Amount Submitted (481-HA), Percentage Sales Tax Amount Submitted (482-GE), Incentive Amount Submitted (438-E3), Other Amount Claimed (48-H9). For service claim request, field represents a sum of Professional Services Fee Submitted (477-BE), Flat Sales Tax Amount Submitted (481-HA), Percentage Sales Tax Amount Submitted (482-GE), Other Amount Claimed (48-H9). This field is EPS only!!!"
    type: sum
    sql: CASE WHEN (${bi_demo_sales.adjudicated_flg} = 'Y' or ${bi_demo_sales.sold_flg} = 'Y')  THEN ${TABLE}.TX_TP_SUBMIT_DETAIL_GROSS_AMOUNT_DUE END ;;
    value_format: "$#,##0.00;($#,##0.00)"
  }
  ...
  measure: sum_sales_tx_tp_submit_detail_ingredient_cost {
    ...
    description: "Submitted product component cost of the dispensed prescription. This amount is included in the 'Gross Amount Due' (430-DU). This field is EPS only!!!"
    type: sum
    sql: CASE WHEN (${bi_demo_sales.adjudicated_flg} = 'Y' or ${bi_demo_sales.sold_flg} = 'Y')  THEN ${TABLE}.TX_TP_SUBMIT_DETAIL_INGREDIENT_COST END ;;
    value_format: "$#,##0.00;($#,##0.00)"
  }
  ...
  measure: sum_sales_tx_tp_submit_detail_patient_paid_amount {
    ...
    description: "Total amount the pharmacy received from the patient for the prescription dispensed. This field is EPS only!!!"
    type: sum
    sql: CASE WHEN (${bi_demo_sales.adjudicated_flg} = 'Y' or ${bi_demo_sales.sold_flg} = 'Y')  THEN ${TABLE}.TX_TP_SUBMIT_DETAIL_PATIENT_PAID_AMOUNT END ;;
    value_format: "$#,##0.00;($#,##0.00)"
  }
  ...
  measure: sum_sales_tx_tp_submit_detail_flat_sales_tax_amount {
    ...
    description: "Flat sales tax submitted for prescription. This amount is included in the 'Gross Amount Due' (430-DU). This field is EPS only!!!"
    type: sum
    sql: CASE WHEN (${bi_demo_sales.adjudicated_flg} = 'Y' or ${bi_demo_sales.sold_flg} = 'Y')  THEN ${TABLE}.TX_TP_SUBMIT_DETAIL_FLAT_SALES_TAX_AMOUNT END ;;
    value_format: "$#,##0.00;($#,##0.00)"
  }
...
############################################################################ Sets ############################################################
  ...
  set: sales_tx_tp_submit_detial_dimension_candidate_list {
    fields: [
      ...
      tx_tp_submit_detail_prescriber_identifier_deidentified,
      tx_tp_submit_detail_cardholder_identifier_deidentified,
      ...
      tx_tp_submit_detail_group_identifier_deidentified,
