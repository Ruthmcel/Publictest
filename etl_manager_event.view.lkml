view: etl_manager_event {
  sql_table_name: ETL_MANAGER.EVENT ;;
  ...
  dimension: refresh_frequency {
    ...
    description: "D-Daily, H-Hourly, M-Monthly, W-Weekly, MM-Minute, (or) O-Ondemand frequency of execution of the various ETL jobs"
    ...
    case: {
      ...
      when: {
        sql: ${TABLE}.REFRESH_FREQUENCY = 'W' ;;
        label: "Weekly"
      }

      when: {
        sql: ${TABLE}.REFRESH_FREQUENCY = 'MM' ;;
        label: "Minute"
      }

      when: {
        sql: ${TABLE}.REFRESH_FREQUENCY = 'M' ;;
        label: "Monthly"
      }
    }
  }
  ...
  measure: sum_billed_event {
    label: "Total Loads Billed"
    description: "Displays the total loads billed for a given event/customer, irrespectie of the load type."
    type: number
    sql: (COUNT(DISTINCT(${event_id}))*9) ;;
    value_format: "$#,##0"
  }
}
