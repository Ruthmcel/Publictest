view: rpt_cal_this_year_last_year_sold_active {
  derived_table: {
    sql: SELECT
      ...
      WHERE {% condition rx_tx_active.this_year_last_year_filter %} 'Yes-Sold' {% endcondition %}         -- Will ensure records are fetched from report calendar only when LY analysis is performed on the Sold Date
      AND {% condition rx_tx_active.sold_date_filter %} CALENDAR_DATE {% endcondition %}
