view: sales_eps_tx_tp_other_payer_amount {
  ...
  dimension: primary_key {
    ...
    primary_key: yes
    sql: ${chain_id} ||'@'|| ${nhin_store_id} ||'@'|| ${tx_tp_other_payer_amount_id} ||'@'|| ${sales.sold_flg} ||'@'|| ${sales.adjudicated_flg} ||'@'|| ${report_calendar_global.type}  ;; #ERXLPS-1649
  }
  ...
  measure: tx_tp_other_payer_amount {
    ...
    type: sum
    sql: CASE WHEN (${sales.adjudicated_flg} = 'Y' or ${sales.sold_flg} = 'Y') AND ${report_calendar_global.type} = 'TY' THEN ${TABLE}.tx_tp_other_payer_amount END ;;
