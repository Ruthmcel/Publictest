view: bi_demo_prescriber_unique {
  ...
  ## 3)  ALL FIELDS IN THIS VIEW ARE HIDDEN. The bi_demo_prescriber VIEW references all the fields in this view, from the bi_demo_prescriber VIEW file. THe JOIN is enforced when dimensions and measures are chosen. This Design was implemented so that any current/legacy reports using that view would not be broken.

  ## 4) prescriber_unique VIEW for bi_demo has additional join to Report Temp to eliminate the scan of all chains in the PRESCRIBER table, since the Chain ID template filter does not exist
      ...
      FROM  (
              ...
              FROM (
                      ...
                      FROM EDW.D_PRESCRIBER P
                        JOIN REPORT_TEMP.BI_DEMO_CHAIN_MAPPING C ON P.CHAIN_ID = C.CHAIN_ID
