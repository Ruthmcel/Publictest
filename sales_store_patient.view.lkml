view: sales_store_patient {
  ...
  measure: patient_total_price_amount {
    ...
    sql: CASE WHEN ${report_calendar_global.type} = 'TY' THEN ${TABLE}.PATIENT_TOTAL_PRICE_AMOUNT END ;;
  }
  ...
  measure: patient_total_prescription_count {
    ...
    sql: CASE WHEN ${report_calendar_global.type} = 'TY' THEN ${TABLE}.PATIENT_TOTAL_PRESCRIPTION_COUNT END ;;
  }
  ...
  measure: store_patient_count_central_patient_explore {
    ...
    type: count_distinct
    sql: CASE WHEN ${report_calendar_global.type} = 'TY' THEN ${chain_id} ||'@'|| ${nhin_store_id} ||'@'|| ${rx_com_id} END ;;
    value_format: "#,##0;(#,##0)"
  }

  measure: store_patient_count_ly {
    label: "LY Store Patient Count"
    description: "Total Store Patients for Last Year based on the period selected"
    type: count_distinct
  ...
  measure: patient_count_variance {
    label: "TY/LY Store Patient Count Variance"
    description: "Increase/Decrease of Total Store Patient Count compared to last year. Calculation Used: (Store Patient Count - LY Store Patient Count)."
    type: number
    sql: ${store_patient_count} - ${store_patient_count_ly} ;;
    value_format: "#,##0;(#,##0)"
  }

  measure: patient_count_variance_pct {
    label: "TY/LY Store Patient Count Variance %"
    description: "Percentage Increase/Decrease of Total Store Patient Count compared to last year. Calculation Used: (Store Patient Count - LY Store Patient Count)/LY Store Patient Count."
    type: number
    sql: CAST((${store_patient_count} - ${store_patient_count_ly}) AS DECIMAL(17,4))/NULLIF(ABS(${store_patient_count_ly}),0) ;;
    value_format: "00.00%"
  }

  #[ERXLPS-6323]
  measure: patient_count_age_65_plus {
    label: "Store Patient Count (Age 65+)"
    description: "Total Patients whose age is 65 and above. Patient Age is calculated at the time of report run."
    type: count_distinct
    sql: CASE WHEN ${report_calendar_global.type} = 'TY' AND ${store_patient_age} >= 65 THEN ${unique_key} END ;;
    value_format: "#,##0;(#,##0)"
  }
