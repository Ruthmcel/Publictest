#ERXLPS-2455
view: bi_demo_sales_eps_tx_tp_transmit_queue_latest_yesno {
  derived_table: {
    sql: select chain_id,
                nhin_store_id,
  ...
  dimension: unique_key {
    ...
    #Added tx_tp_id to UK. This is required now as we are integrating TP Transmit Queue into Sales explore. This will make sure sum_distinct is applied correctly while using measures from different measures.
        sql: ${chain_id} ||'@'|| ${nhin_store_id} ||'@'|| ${tx_tp_id} ||'@'|| ${tx_tp_transmit_queue_id} ||'@'|| ${bi_demo_sales.sold_flg} ||'@'|| ${bi_demo_sales.adjudicated_flg} ;;  }
