view: rpt_cal_this_year_last_year_filled {
  derived_table: {
    sql: SELECT
      ...
      WHERE {% condition rx_tx.this_year_last_year_filter %} 'Yes-Filled' {% endcondition %}    -- Will ensure records are fetched from report calendar only when LY analysis is performed on the Filled Date
      AND {% condition rx_tx.filled_date_filter %} CALENDAR_DATE {% endcondition %}
