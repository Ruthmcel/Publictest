view: etl_manager_event_job {
  sql_table_name: ETL_MANAGER.EVENT_JOB ;;
  ...
  measure: sum_unique_job_count {
    label: "Total Unique jobs"
    description: "Displays the total unique jobs for any given processing window, irrespective of the total events or chain processed in a given event"
    type: number
    sql: COUNT(DISTINCT(${job_name})) ;;
    value_format: "#,##0"
  }
}
