view: sales_eps_tx_tp_dur {
  ...
  dimension: primary_key {
    ...
    type: number
    primary_key: yes
    sql: ${chain_id} ||'@'|| ${nhin_store_id} ||'@'|| ${tx_tp_dur_id} ||'@'|| ${sales.sold_flg} ||'@'|| ${sales.adjudicated_flg} ||'@'|| ${report_calendar_global.type}  ;; #ERXLPS-1649
  }
  ...
  measure: sum_tx_tp_dur_previous_fill_quantity {
    ...
    type: sum
    sql: CASE WHEN (${sales.adjudicated_flg} = 'Y' or ${sales.sold_flg} = 'Y') AND ${report_calendar_global.type} = 'TY' THEN ${TABLE}.TX_TP_DUR_PREVIOUS_FILL_QUANTITY END ;;
    value_format: "###0.00"
  }
