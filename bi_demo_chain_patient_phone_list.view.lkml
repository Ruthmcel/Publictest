view: bi_demo_chain_patient_phone_list {
  derived_table: {
      sql: Select chain_id,rx_com_id,
        ...
        WHERE PAD.chain_id IN (SELECT DISTINCT CHAIN_ID FROM REPORT_TEMP.BI_DEMO_CHAIN_STORE_MAPPING WHERE {% condition bi_demo_store.bi_demo_nhin_store_id %} NHIN_STORE_ID_BI_DEMO_MAPPING {% endcondition %})   -- Required for performance reasons and to avoid scanning all chain/store records
        and PATIENT_ADDRESS_DEACTIVATE_DATE is null and PATIENT_ADDRESS_DELETED = 'N'
