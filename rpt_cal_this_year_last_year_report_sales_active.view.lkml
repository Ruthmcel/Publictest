view: rpt_cal_this_year_last_year_report_sales_active {
  derived_table: {
    sql: SELECT
      ...
      WHERE {% condition rx_tx_active.this_year_last_year_filter %} 'Yes-ReportableSales' {% endcondition %}     -- Will ensure records are fetched from report calendar only when LY analysis is performed on the Reportable Sales Date
      AND {% condition rx_tx_active.reportable_sales_date_filter %} CALENDAR_DATE {% endcondition %}
