...
view: bi_demo_sales_tx_tp {
  derived_table: {
    sql: SELECT *
            ...
            FROM (SELECT CHAIN_ID,
                        ...
                        AND ( (TX_TP_PAID_STATUS IN (1, 2, 3, 4, 5) OR TX_TP_SUBMIT_TYPE = 'D')  -- added status 3 to match SASD logic, however, do not agree this status should live on tx_tp record
                              OR
  ...
  dimension: last_counter_final_copay {
    ...
    type: number
    sql: CASE WHEN ${bi_demo_sales.financial_category} IN ('T/P - CREDIT','CASH - CREDIT') THEN (${TABLE}.TX_TP_LAST_COUNTER_FINAL_COPAY * -1) WHEN ${bi_demo_sales.financial_category} IN ('T/P - FILLED','CASH - FILLED') THEN (${TABLE}.TX_TP_LAST_COUNTER_FINAL_COPAY) END ;;
  ...
  dimension: final_copay {
    ...
    type: number
    sql: CASE WHEN ${bi_demo_sales.financial_category} IN ('T/P - CREDIT','CASH - CREDIT') THEN (${TABLE}.TX_TP_FINAL_COPAY * -1) WHEN ${bi_demo_sales.financial_category} IN ('T/P - FILLED','CASH - FILLED') THEN (${TABLE}.TX_TP_FINAL_COPAY) END ;;
    value_format: "$#,##0.00;($#,##0.00)"
  }
  ...
  dimension: final_price {
    ...
    sql: CASE WHEN ${bi_demo_sales.financial_category} IN ('T/P - CREDIT','CASH - CREDIT') THEN (${TABLE}.TX_TP_FINAL_PRICE * -1) WHEN ${bi_demo_sales.financial_category} IN ('T/P - FILLED','CASH - FILLED') THEN (${TABLE}.TX_TP_FINAL_PRICE) END ;;
    value_format: "$#,##0.00;($#,##0.00)"
  }
  ...
  dimension: copay_override_amount {
    ...
    type: number
    #     sql: ${TABLE}.TX_TP_FINAL_PRICE
    sql: CASE WHEN ${bi_demo_sales.financial_category} IN ('T/P - CREDIT','CASH - CREDIT') THEN (${TABLE}.TX_TP_COPAY_OVERRIDE_AMOUNT * -1) WHEN ${bi_demo_sales.financial_category} IN ('T/P - FILLED','CASH - FILLED') THEN (${TABLE}.TX_TP_COPAY_OVERRIDE_AMOUNT) END ;;
    value_format: "$#,##0.00;($#,##0.00)"
  }
  ...
  dimension: copay_overridden_amount {
    ...
    type: number
    sql: CASE WHEN ${bi_demo_sales.financial_category} IN ('T/P - CREDIT','CASH - CREDIT') THEN (${TABLE}.TX_TP_OVERRIDDEN_COPAY_AMOUNT * -1) WHEN ${bi_demo_sales.financial_category} IN ('T/P - FILLED','CASH - FILLED') THEN (${TABLE}.TX_TP_OVERRIDDEN_COPAY_AMOUNT) END ;;
    value_format: "$#,##0.00;($#,##0.00)"
  }
  ...
  dimension: write_off {
    ...
    hidden: yes
    sql: CASE WHEN ${bi_demo_sales.financial_category} IN ('T/P - CREDIT','CASH - CREDIT') THEN (${TABLE}.TX_TP_WRITE_OFF_AMOUNT * -1) WHEN ${bi_demo_sales.financial_category} IN ('T/P - FILLED','CASH - FILLED') THEN (${TABLE}.TX_TP_WRITE_OFF_AMOUNT) END ;;
    value_format: "$#,##0.00;($#,##0.00)"
    html: {% if value < 0 %}
  ...
  dimension: net_due {
    ...
    # referenced in sales view files to avoid running into non unique collision error
    hidden: yes
    sql: CASE WHEN ${bi_demo_sales.financial_category} IN ('T/P - CREDIT','CASH - CREDIT') THEN (${TABLE}.TX_TP_BALANCE_DUE_FROM_TP * -1) WHEN ${bi_demo_sales.financial_category} IN ('T/P - FILLED','CASH - FILLED') THEN (${TABLE}.TX_TP_BALANCE_DUE_FROM_TP) END ;;
    value_format: "$#,##0.00;($#,##0.00)"
    html: {% if value < 0 %}
        <font color="red">{{ rendered_value }}
      {% else %}
  ...
  dimension: final_tax {
    ...
    # referenced in sales view files to avoid running into non unique collision error
    hidden: yes
    sql: CASE WHEN {% condition bi_demo_sales.sales_rxtx_payor_summary_detail_analysis %} 'SUMMARY' {% endcondition %} AND  ${bi_demo_sales.financial_category} IN ('T/P - CREDIT','CASH - CREDIT') THEN (${TABLE}.MAX_TX_TP_FINAL_TAX * -1)
              WHEN {% condition bi_demo_sales.sales_rxtx_payor_summary_detail_analysis %} 'SUMMARY' {% endcondition %} AND  ${bi_demo_sales.financial_category} IN ('T/P - FILLED','CASH - FILLED') THEN (${TABLE}.MAX_TX_TP_FINAL_TAX)
              WHEN {% condition bi_demo_sales.sales_rxtx_payor_summary_detail_analysis %} 'DETAIL' {% endcondition %} AND ${bi_demo_sales.financial_category} IN ('T/P - CREDIT','CASH - CREDIT') THEN (${TABLE}.TX_TP_FINAL_TAX * -1)
              WHEN {% condition bi_demo_sales.sales_rxtx_payor_summary_detail_analysis %} 'DETAIL' {% endcondition %} AND ${bi_demo_sales.financial_category} IN ('T/P - FILLED','CASH - FILLED') THEN (${TABLE}.TX_TP_FINAL_TAX)
         END;;
    value_format: "$#,##0.00;($#,##0.00)"
  }
  ...
  measure: count_tx_tp {
    ...
    sql: count(DISTINCT(${TABLE}.CHAIN_ID ||'@'|| ${TABLE}.NHIN_STORE_ID ||'@'|| ${TABLE}.RX_TX_ID ||'@'|| ${TABLE}.TX_TP_ID)) ;;
    value_format: "#,##0"
  }
  ...
  ################################################################################### End of ERXLPS-645 - GenRx Missing Dimensions/Measures adding to Sales Explore #########################################################################
  ...
  dimension: tx_tp_primary_paid_status {
    ...
    type: string
    sql: CASE WHEN TX_TP_PRIMARY_PAID_STATUS IN (1,2,5,6) THEN 'PAID' WHEN TX_TP_PRIMARY_PAID_STATUS IN (4) THEN 'CREDIT' WHEN TX_TP_PRIMARY_PAID_STATUS IN (3) THEN 'REJECT' ELSE 'NO STATUS' END ;;
  ...
  dimension: tx_tp_secondary_paid_status {
    ...
    type: string
    sql: CASE WHEN TX_TP_SECONDARY_PAID_STATUS IN (1,2,5,6) THEN 'PAID' WHEN TX_TP_SECONDARY_PAID_STATUS IN (4) THEN 'CREDIT' WHEN TX_TP_SECONDARY_PAID_STATUS IN (3) THEN 'REJECT' ELSE 'NO STATUS' END ;;
    suggestions: ["PAID", "CREDIT", "REJECT", "NO STATUS"]
  }
  ...
  dimension: tx_tp_tertiary_paid_status {
    ...
    type: string
    sql: CASE WHEN TX_TP_TERTIARY_PAID_STATUS IN (1,2,5,6) THEN 'PAID' WHEN TX_TP_TERTIARY_PAID_STATUS IN (4) THEN 'CREDIT' WHEN TX_TP_TERTIARY_PAID_STATUS IN (3) THEN 'REJECT' ELSE 'NO STATUS' END ;;
    suggestions: ["PAID", "CREDIT", "REJECT", "NO STATUS"]
  }
  ...
  dimension: tx_net_sales {
    ...
    type: number
    hidden: yes
    sql: CASE WHEN ${bi_demo_sales.financial_category} IN ('T/P - CREDIT','CASH - CREDIT') THEN (${TABLE}.TX_SUMMARY_NET_SALES * -1) WHEN ${bi_demo_sales.financial_category} IN ('T/P - FILLED','CASH - FILLED') THEN (${TABLE}.TX_SUMMARY_NET_SALES) END ;;
    value_format: "$#,##0.00;($#,##0.00)"
  }
}
