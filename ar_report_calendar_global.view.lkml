view: ar_report_calendar_global {
  derived_table: {
    sql: with
      input_parameters
      as (select rpt_start_date,
                 ...
                 case when {% date_end report_period_filter %} is null then duration + 1
                      else duration
                 end duration,
                 offset,
                 date_range_end
            from (select rpt_start_date,
                    ...
                    from (select nvl({% date_start report_period_filter %},
                                     to_date('1900-01-01')) rpt_start_date,
                                 nvl({% date_end report_period_filter %},
                                 dateadd('day', 1, current_date())) rpt_end_date,
                                 replace(replace(regexp_replace(initcap({% parameter analysis_calendar_filter %}), '.* - '), 'Date Range', 'Day'), 'Specific Year', 'Year') analysis_calendar_filter
                         )
      ...
      as (select fiscal_date.*,
            ...
            from (select fiscal_date.chain_id calendar_owner_chain_id,
                         ...
                         case when regexp_replace(initcap({% parameter analysis_calendar_filter %}), '.* - ') = 'Week'
                   ...
                   where {% condition ar_chain.chain_id %} fiscal_date.chain_id {% endcondition %}
                     and upper(regexp_replace(initcap({% parameter analysis_calendar_filter %}), ' - .*')) = fiscal_date.calendar_type
                     -- and regexp_replace(initcap({% parameter analysis_calendar_filter %}), ' - .*') = 'Fiscal'
      ...
      as (select *
           ...
           where calendar_date = current_date()
             and {% condition ar_chain.chain_id %} fiscal_calendar.chain_id {% endcondition %}
         ),
      base_fiscal_date
      as (select fiscal_calendar.*,
           ...
           where current_fiscal_date.chain_id = fiscal_calendar.chain_id
             and 1 = case when     input_parameters.date_range_end < current_fiscal_date.calendar_date
                               and regexp_replace(initcap({% parameter analysis_calendar_filter %}), '.* - ') = 'Specific Year'
                          ...
                          when     input_parameters.date_range_end < current_fiscal_date.calendar_date
                               and regexp_replace(initcap({% parameter analysis_calendar_filter %}), '.* - ') = 'Year'
                               and fiscal_calendar.fiscal_year = current_fiscal_date.fiscal_year - input_parameters.offset
                               and fiscal_calendar.calendar_date = fiscal_calendar.fiscal_year_end_date
                          then 1
                          when     input_parameters.date_range_end < current_fiscal_date.calendar_date
                               and regexp_replace(initcap({% parameter analysis_calendar_filter %}), '.* - ') = 'Quarter'
                          ...
                          then 1
                          when     input_parameters.date_range_end < current_fiscal_date.calendar_date
                               and regexp_replace(initcap({% parameter analysis_calendar_filter %}), '.* - ') = 'Month'
                          ...
                          then 1
                          when     input_parameters.date_range_end < current_fiscal_date.calendar_date
                               and regexp_replace(initcap({% parameter analysis_calendar_filter %}), '.* - ') = 'Week'
                           ...
                           then 1
                          when     replace(regexp_replace(initcap({% parameter analysis_calendar_filter %}), '.* - '), 'Date Range', 'Day') = 'Day'
      ...
      as (select fiscal_calendar.*,
           ...
           where fiscal_calendar.chain_id = base_fiscal_date.chain_id
            -- and regexp_replace(initcap({% parameter analysis_calendar_filter %}), ' - .*') = 'Fiscal'
             and 1 = case when     replace(regexp_replace(initcap({% parameter analysis_calendar_filter %}), '.* - '), 'Specific Year', 'Year') = 'Year'
                          ...
                          then 1
                          when     regexp_replace(initcap({% parameter analysis_calendar_filter %}), '.* - ') = 'Quarter'
                               ...
                               and fiscal_calendar.calendar_date < (dateadd('day', 1, base_fiscal_date.calendar_date))
                          then 1
                          when     regexp_replace(initcap({% parameter analysis_calendar_filter %}), '.* - ') = 'Month'
                               ...
                               and fiscal_calendar.calendar_date < (dateadd('day', 1, base_fiscal_date.calendar_date))
                          then 1
                          when     regexp_replace(initcap({% parameter analysis_calendar_filter %}), '.* - ') = 'Week'
      ...
      as (select distinct
                 ...
                 case when replace(regexp_replace(initcap({% parameter analysis_calendar_filter %}), '.* - '), 'Specific Year', 'Year') = 'Year'
                      then null
                      when regexp_replace(initcap({% parameter analysis_calendar_filter %}), '.* - ') = 'Quarter'
           ...
           -- where regexp_replace(initcap({% parameter analysis_calendar_filter %}), ' - .*') = 'Fiscal'
            where initcap({% parameter this_year_last_year_filter %}) = 'Yes'
      ),
      fiscal_ly_calendar
      as (select ly_fiscal_calendar.*,
            ...
            from (select ly_fiscal_calendar.chain_id calendar_owner_chain_id,
                   ...
                   where ly_fiscal_calendar.chain_id = fiscal_ly_range.chain_id
                     ...
                     and (   (    replace(regexp_replace(initcap({% parameter analysis_calendar_filter %}), '.* - '), 'Specific Year', 'Year') = 'Year'
                              and 1 = 1
                             )
                          or (    regexp_replace(initcap({% parameter analysis_calendar_filter %}), '.* - ') = 'Quarter'
                              and ly_fiscal_calendar.last_year_fiscal_quarter_of_year = fiscal_ly_range.join_key
                             )
                          or (    regexp_replace(initcap({% parameter analysis_calendar_filter %}), '.* - ') = 'Month'
                              and ly_fiscal_calendar.last_year_fiscal_month_of_year = fiscal_ly_range.join_key
                             )
                          or (    regexp_replace(initcap({% parameter analysis_calendar_filter %}), '.* - ') = 'Week'
                              and ly_fiscal_calendar.last_year_fiscal_week_of_year = fiscal_ly_range.join_key
                             )
                          or (    replace(regexp_replace(initcap({% parameter analysis_calendar_filter %}), '.* - '), 'Date Range', 'Day') = 'Day'
                              and 1 = 1
                             )
                         )
                     and initcap({% parameter this_year_last_year_filter %}) = 'Yes'
                 ) ly_fiscal_calendar
      ...
      as (select fiscal_ly_calendar.calendar_owner_chain_id,
                 ...
                 case when fiscal_ty_calendar.calendar_date is null
                      then max(fiscal_ty_calendar.calendar_date) over(partition by case when replace(regexp_replace(initcap({% parameter analysis_calendar_filter %}), '.* - '), 'Specific Year', 'Year') = 'Year'
           ...
           -- where regexp_replace(initcap({% parameter analysis_calendar_filter %}), ' - .*') = 'Fiscal'
           where initcap({% parameter this_year_last_year_filter %}) = 'Yes'
             and case when regexp_replace(initcap({% parameter analysis_calendar_filter %}), '.* - ') = 'Year' then
                       ...
                       when regexp_replace(initcap({% parameter analysis_calendar_filter %}), '.* - ') = 'Quarter' then
                        case when (    offset_flag = 'N'
                                   and (fiscal_ty_calendar.fiscal_year - 1) >= fiscal_ly_calendar.fiscal_year
                                   and fiscal_ty_calendar.fiscal_quarter_of_year = fiscal_ly_calendar.fiscal_quarter_of_year
                                   and fiscal_ty_calendar.fiscal_month_of_year = fiscal_ly_calendar.fiscal_month_of_year
                       ...
                       when replace(regexp_replace(initcap({% parameter analysis_calendar_filter %}), '.* - '), 'Date Range', 'Day') = 'Day' then
       ...
       where ly.chain_id = cf.chain_id
         ...
         and case when (    ly.offset_flag = 'N'
                        ...
                        and (   (     regexp_replace(initcap({% parameter analysis_calendar_filter %}), '.* - ') = 'Quarter'
                             ...
                             OR (     regexp_replace(initcap({% parameter analysis_calendar_filter %}), '.* - ') = 'Year'
                                  and ly.rnk_year = 1
  ...
  dimension: report_range {
    ...
    description: "The Date Range of the Report Period based on the date range specified in the REPORT PERIOD required filter"
    type: string
