view: chain_patient_summary_info {
  ...
  derived_table: {
    sql: Select CHAIN_ID, RX_COM_ID,PAT_FIRST_FILL_DATE,PAT_LAST_FILLED_DATE,PAT_LAST_VISITED_STORE,PAT_LAST_VISITED_NHIN_STORE_ID,
      ...
      CaSE WHEN DRUG_RNK <=  {% parameter aggregated_medication_name %} then STORE_DRUG_NAME END STORE_DRUG_NAME,
      ...
      WHERE    {% condition chain.chain_id %} TX.CHAIN_ID {% endcondition %} -- Required for performance reasons and to avoid scanning all chain/store records
              AND {% condition chain.chain_id %} RX.CHAIN_ID {% endcondition %}
              AND {% condition chain.chain_id %} PAT.CHAIN_ID {% endcondition %}
              AND {% condition chain.chain_id %} SDRG.CHAIN_ID {% endcondition %}
